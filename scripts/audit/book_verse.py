...
``scripts/lib/ir_normalize.py`` (``verse_blocks`` / ``_run_kind`` /
``_is_lineated_line``).
...
ROOT = Path(os.environ.get("PANCRATIUS_AUDIT_ROOT") or Path(__file__).resolve().parents[2])
...
# and is NOT a verse line. Mirrors ``ir_normalize.VERSE_SHORT_LINE_MAX``; the two
...
def _speaker_turn_re() -> re.Pattern[str]:
    ...
    dialogue prefixes (`ir_normalize._DIALOGUE_PREFIXES`, the dialogue SoT) so the
    ...
    from lib.ir_normalize import _DIALOGUE_PREFIXES  # noqa: PLC0415
...
def _is_wrapped_prose(inlines: list[dict[str, Any]]) -> bool:
    ...
    authored. Mirrors ``ir_normalize._is_wrapped_prose`` (the C2 over-detection
...
def source_right_aligned_words(docx: Path) -> set[str]:
    ...
    sys.path.insert(0, str(ROOT / "scripts"))
    from lib import docx_adapter  # noqa: PLC0415
    ...
    for number, title, slug in _committed_book_meta():
        ...
        with tempfile.TemporaryDirectory(prefix="book-verse-ir-") as td:
            ...
            parsed = import_docx.build_parser().parse_args([
                str(docx), "--kind", "book", "--lang", "ru",
                "--number", str(number), "--slug", slug, "--title", title,
                "--out-content", str(content_root),
            ])
            with contextlib.redirect_stdout(io.StringIO()), contextlib.redirect_stderr(io.StringIO()):
                result = import_docx.run(parsed)
            md_body = _md_body(result.md_path.read_text(encoding="utf-8"))
