...
`scripts/render_downloads.py` when refreshing committed per-work release
artifacts. Sibling directory `scripts/downloads-fonts/` holds the bundled fonts.

## Layout

```
scripts/downloads-templates/
  book.typ        — pandoc template for typst PDF output
  epub.css        — slim stylesheet for EPUB (used by --css)
  README.md       — this file

scripts/downloads-fonts/
...
`render_downloads.py` shells out roughly as follows:

```
pandoc <md> -o <pdf>
  --pdf-engine=typst
  --template scripts/downloads-templates/book.typ
  --pdf-engine-opt=--root=/
...
- The cover-path metadata variable is consumed by the `$if(cover-path)$`
  branch in `book.typ`. Non-book works (poems, projects) currently omit the
  cover and get a plain text title page; if a poem ever ships a cover image
