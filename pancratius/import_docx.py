...
from pancratius.docx_conversion import (
    ConvertedDocx,
    body_asset_ops,
    convert_single_docx,
...
STAGE_ROOT = CACHE_ROOT / "import-stage"
...
def _imports_dir(content_root: Path) -> Path:
    """Where the out-of-bundle provenance manifest lands.

    A temp ``--out-content`` shaped like ``<root>/src/content`` sandboxes to its
    own sibling ``<root>/data/imports`` instead of the real repo.
    """
    try:
        return imports_dir_for_content_root(content_root)
...
def _write_docx_artifact(src: Path, dst: Path) -> None:
    ...
    from pancratius.docx_optimize import optimize_docx

    dst.parent.mkdir(parents=True, exist_ok=True)
    try:
        optimize_docx(src, dst)
    except (OSError, zipfile.BadZipFile, zipfile.LargeZipFile) as exc:
        raise ImportWorkError(f"failed to optimize DOCX {src} -> {dst}") from exc
...
def _apply(request: ImportRequest) -> tuple[ImportResult, WriteReport]:
    ...
    in `WriteReport.refused` with its fatal diagnostics. Raises `ImportWorkError` only for
    invalid input or an unresolvable target. The writer is the only mutator."""
    docx = Path(request.docx).expanduser().resolve()
    if not docx.is_file():
        raise ImportWorkError(f"DOCX not found: {docx}")
    if docx.suffix.lower() != ".docx":
        raise ImportWorkError(f"expected a .docx file: {docx}")
