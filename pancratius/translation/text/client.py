...
The translation pipeline reaches one external service. Rather than add an HTTP
dependency to the deliberately light package core, this wraps ``urllib`` with the
few things the pipeline actually needs: bearer auth from the environment, prompt
caching markers (so a book's read-only reference is billed once at the cache-read
rate for the rest of its chunks), retry/backoff on rate limits, and a typed
``Usage`` so cost accounting reads real per-call token counts back from the API.

Pricing is fetched live from the public ``/models`` endpoint — never hardcoded —
because OpenRouter prices drift.
"""
...
class OpenRouterError(RuntimeError):
    """A translation API call failed (auth, transport, or a malformed reply)."""
...
class ChatMessage:
    ...
    stable prefix (style guide + book reference) is reused across a book's chunks."""
...
class TranslatorClient(Protocol):
    """The slice of the client the pipeline depends on. Typing against this (not
    the concrete class) keeps the pipeline testable with a stub and inverts the
    network dependency."""
...
def _message_payload(message: ChatMessage) -> JsonObject:
    # DeepSeek (the default) caches automatically on a stable prefix, so what
    # actually secures the hits is message ORDER: constant style guide + book
    # reference first, varying instruction last. The explicit breakpoint below is
    # for providers that need a manual marker (e.g. Anthropic, Gemini) and is a
    # harmless no-op for auto-caching ones.
...
class OpenRouterClient:
    ...
    stateless ``complete`` calls the pipeline makes."""
    ...
    def __init__(
        ...
        *,
        ...
        title: str = "Pancratius book translation",
    ) -> None:
    ...
    def from_env(cls) -> OpenRouterClient:
        ...
        if not key:
            raise OpenRouterError(
                "OPENROUTER_API_KEY is not set; export it (e.g. from .env) before translating."
            )
    ...
    def complete(
        ...
        *,
        ...
        if reasoning_max_tokens is not None:
            ...
            # thinking and return empty content (the ds-flash runaway).
        ...
        if content is None:
            ...
            # — a best-effort revise keeps its draft; a draft fails its checks loudly.
            content = ""
        if not isinstance(content, str):
            raise OpenRouterError("response message had non-string content")
        return Completion(text=content, usage=_usage_from(body.get("usage") or {}), model=model)
