...
with an `audit` verb that shells to `mise run audit:repo`, plus a top-level `check` verb —
...
def build_parser() -> argparse.ArgumentParser:
    ...
    sub.add_parser("check", help="Type-check the repository (FORBIDDEN — belongs to mise).")
    site = sub.add_parser("site", help="Site proxy (FORBIDDEN — verify lives in mise).")
    site_sub = site.add_subparsers(dest="verb")
    site_sub.add_parser("audit", help="Proxy to mise run audit:repo (FORBIDDEN).")
    return parser
