...
def _translated_docx_paths() -> list[Path]:
...
def _docx_issues(path: Path) -> list[TranslatedDocxIssue]:
    try:
        ...
        messages = [str(exc)]
    return [TranslatedDocxIssue(path, message) for message in messages]


def main() -> int:
    paths = _translated_docx_paths()
    issues = [
        issue
        for path in paths
        for issue in _docx_issues(path)
    ]
    if issues:
        print(f"FAIL: {len(issues)} translated DOCX transfer artifact issue(s)", file=sys.stderr)
        for issue in issues:
            print(f"  {issue.path}: {issue.message}", file=sys.stderr)
        return 1
    print(
        f"checked {len(paths)} translated DOCX artifact(s); "
