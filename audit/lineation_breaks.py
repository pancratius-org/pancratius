...
Lineated surfaces:
  ...
  1. BOOK / project `<div class="verse-block">` wrappers — every NON-FINAL line of
     a stanza (a stanza is a maximal run of non-blank lines, with `***` its own
     one-line stanza) must end with exactly two trailing spaces.
...
def _check_verse_blocks(path: Path, text: str, failures: list[str]) -> int:
    checked = 0
    for match in VERSE_RE.finditer(text):
        ...
        for idx in _check_stanza_lines(lines):
            ...
            failures.append(
                f"{path}:{line_no}: verse-block line lost its two-space hard break: "
                f"{lines[idx]!r}"
            )
    return checked
...
def main() -> int:
    ...
    for path in sorted((CONTENT / "books").glob("*/*.md")) + sorted((CONTENT / "projects").glob("*/*.md")):
        verse_blocks += _check_verse_blocks(path, path.read_text(encoding="utf-8"), failures)
    ...
    print(
        f"checked {verse_blocks} verse-blocks + {len(poems)} poem bodies + "
        f"{len(verse_pages)} verse-register pages; all hard breaks intact"
    )
    return 0
