...
*shallow* or *missing*, and lock behaviour the build CLAIMS but does not yet pin:
  ...
  - `request_from_namespace` is a FULL 1:1 flag→field map (the door's own claim),
    not just the four fields the existing test spot-checks.
...
# ===========================================================================
# 1) request_from_namespace — FULL 1:1 flag→field map (door's own claim)
...
def _imports_dir_for(content_root: Path) -> Path:
    # mirrors import_docx._imports_dir: content_root.parents[1] / data / imports
...
def test_data_graph_generate_extras_gate_reproduced(capsys: pytest.CaptureFixture[str]) -> None:
    """With the `graph` extra genuinely absent, `data graph generate` exits 1 with
    the `uv sync --extra graph` hint and prints NO traceback (no mocks)."""
    rc = _exit_code(["data", "graph", "generate"])
    err = capsys.readouterr().err
    assert rc == 1
...
def test_data_embed_generate_extras_gate_reproduced(capsys: pytest.CaptureFixture[str]) -> None:
    """With the `embed` extra genuinely absent, `data embed generate` exits 1 with
    the `uv sync --extra embed` hint and prints NO traceback (no mocks)."""
    rc = _exit_code(["data", "embed", "generate"])
    err = capsys.readouterr().err
    assert rc == 1
...
def conceptosphere_module() -> Iterator[ModuleType]:
    """Import scripts/conceptosphere.py with its heavy deps faked by MagicMock, so
    ...
    sys.modules.pop("conceptosphere", None)
    try:
        yield _il.import_module("conceptosphere")
    finally:
...
def test_generate_graph_none_runs_both_projections(
    ...
    rc = cs.generate_graph(only=None)
    assert rc == 0
...
def test_generate_graph_only_runs_single_projection(
    ...
    monkeypatch.setattr(cs, "run_concepts_mode", lambda *a, **k: ran.append("concepts") or 0)
    monkeypatch.setattr(cs, "run_books_mode", lambda *a, **k: ran.append("books") or 0)
    assert cs.generate_graph(only=only) == 0
    assert ran == expected


def test_generate_graph_first_nonzero_rc_propagates(
    conceptosphere_module: ModuleType, monkeypatch: pytest.MonkeyPatch
) -> None:
    """A failed concepts projection must NOT skip books, and a nonzero rc must
    propagate (so the door collapses it to a failure exit)."""
    cs = conceptosphere_module
    ran: list[str] = []
    monkeypatch.setattr(cs, "process_corpus", lambda log: object())
    monkeypatch.setattr(cs, "run_concepts_mode", lambda *a, **k: ran.append("concepts") or 5)
    monkeypatch.setattr(cs, "run_books_mode", lambda *a, **k: ran.append("books") or 0)
    rc = cs.generate_graph(only=None)
    assert ran == ["concepts", "books"], "books must still run after a concepts failure"
    assert rc != 0, "a nonzero projection rc must propagate"
