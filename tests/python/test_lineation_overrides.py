...
load. In the fold pass, a prose-pinned paragraph never enters a lineation unit; the `lineated`
...
def _short_run() -> list[ir.Block]:
    # A heading-opened run of short source rows the fold pass folds (the inference path).
    return [
...
def _run_pipeline(
    ...
    overrides: dict[int, ir.LineationRegister] | None = None,
) -> list[ir.Block]:
    doc = run(
...
def test_fate_assertion_catches_an_unhonored_override() -> None:
    blocks: list[ir.Block] = [ir.LineatedBlock(
        stanzas=[[ir.Line([ir.Text("строка")])]],
        source_span=ir.SourceSpan(start=5, end=7),
    )]
    with pytest.raises(RuntimeError, match="not honored"):
        check_overrides_held(blocks, {6: "prose"})
    check_overrides_held(blocks, {99: "prose"})   # disjoint ordinal passes


def test_fate_assertion_sees_a_fold_inside_a_quote_wrapper() -> None:
    # A prose-pinned ordinal hidden inside a QuoteBlock-wrapped lineated fold is still
    # a violation; a plain prose MEMBER paragraph of a QuoteBlock is not.
    wrapped: list[ir.Block] = [ir.QuoteBlock(blocks=[ir.LineatedBlock(
        stanzas=[[ir.Line([ir.Text("строка")])]],
        source_span=ir.SourceSpan(start=5, end=7),
    )])]
    with pytest.raises(RuntimeError, match="not honored"):
        check_overrides_held(wrapped, {6: "prose"})
    member: list[ir.Block] = [ir.QuoteBlock(blocks=[
        ir.Paragraph(inlines=[ir.Text("проза")], source_span=ir.SourceSpan(start=6, end=6)),
    ])]
    check_overrides_held(member, {6: "prose"})   # prose member paragraphs never count
