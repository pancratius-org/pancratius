...
def _recipe(selector: recipes.Selector | None = None, books: tuple = ("57",)) -> recipes.Recipe:
    selector = selector or recipes.AllVotable()
...
from intent_ai.identity import LineId  # noqa: E402
from intent_ai.teacher.tasks import ItemSpec  # noqa: E402
...
def test_route_and_ingest_stamp_holdout_from_the_recipe_membership():
    ...
    from intent_ai.teacher.recipes import _holdout_members
...
def test_build_page_sizes_oversized_runs_for_text_recipes_too(tmp_path):
    ...
    for s in sized:
        ordinals = [line_id.src_ordinal for line_id in s.votable]
        assert max(ordinals) - min(ordinals) <= PAGE_SPAN_CAP
