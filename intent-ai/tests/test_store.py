...
import pytest
from intent_ai import artifact, store
from intent_ai.identity import ExperimentId, LineId
...
def test_annotation_rows_load_from_committed_store():
    ...
    assert all(LineId.from_key(k).as_key() == k for k in contested)
...
@pytest.mark.parametrize("value", ["history", "history/source-v2"])
def test_experiment_identity_cannot_escape_the_active_namespace(value):
