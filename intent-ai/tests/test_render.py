...
prove per-page labeling, encoding, and source-ordinal slicing. A multi-page region yields one image
per page; an empty region fails loud rather than silently becoming a text task."""
...
def test_span_is_min_max_source_ordinal():
    ids = [LineId.mapped("ru", "57", ordinal, 0) for ordinal in (10, 14, 12)]
    spec = _spec("b57-r0", ids)
    rp = _stub_renderer()
...
def test_empty_region_fails_loud():
    spec = _spec("b57-r0", [], votable=[])
    compose = render.make_compositor(_stub_renderer(), docx_for=_docx_for_stub([]), page_boundaries=_NO_BREAKS)
    with pytest.raises(render.RenderError, match="no source lines"):
        compose([spec])
...
def test_region_mixing_book_or_lang_fails_loud():
    # render is a trust boundary: a region whose lines span two books cannot anchor one
