...
def test_context_listing_uses_opaque_keys():
    ...
    source_key = re.escape(f"{first_vote.src_ordinal}.{first_vote.sub}")
    assert not re.search(rf"(?m)^  {source_key}\s", context)  # no src-ordinal listing key
...
def test_manifest_roundtrips_retired_source_lineage():
    raw = {
        "by_key": {},
        "text_hash_by_key": {},
        "item_by_key": {},
        "retired_by_key": {"L001": ["en", "01", 12, 0]},
        "retired_text_hash_by_key": {"L001": "abc"},
    }
    manifest = tasks.TaskManifest.from_dict(raw)
