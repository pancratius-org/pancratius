...
from ..wire import mapping, number
...
class PriceTable:
    ...
    def from_dict(cls, raw: JsonObject) -> PriceTable:
        ...
        rows = mapping(raw.get("models", {}), field="prices.models")
        models: dict[ModelId, Price] = {}
        for model, value in rows.items():
            row = mapping(value, field=f"prices.models.{model}")
            models[model] = (
                number(row["prompt"], field=f"prices.models.{model}.prompt"),
                number(row["completion"], field=f"prices.models.{model}.completion"),
            )
