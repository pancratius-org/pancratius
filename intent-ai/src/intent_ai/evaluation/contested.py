...
def evaluate(records: RecordsByBook, labelset: LabelSet,
    ...
    predictions: LabelByLine = {line_id: decision.label for line_id, decision in oof.items()}
    return evaluate_predictions(labelset, votes, contested, predictions)


def evaluate_predictions(
    labelset: LabelSet,
    votes: PanelVotes,
    contested: LabelByLine,
    predictions: LabelByLine,
) -> ContestedResult:
    """Judge an already-produced student prediction map on the contested slice."""
    labeled_predictions = {
        label.id: predictions[label.id]
        for label in labelset.labels
        if label.id in predictions
    }
    scorable = [k for k in contested if k in labeled_predictions]
    ...
    return ContestedResult(
        ...
        student=balanced(yt, [labeled_predictions[k] for k in scorable]),
        rows=score_readers(contested, labeled_predictions, votes),
    )
