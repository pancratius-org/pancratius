...
from ..identity import ExperimentId, JsonObject, Label, LineId
from ..instrument import E1_V2
from ..recon import Tier0, suspicion_v0
from ..wire import integer, mapping
from .datasets import INDEPENDENT_TRUTH

WORKING_SET = E1_V2.working_set
FROZEN_SET = E1_V2.frozen_set
...
class CorpusRecon:
    ...
    @classmethod
    def from_experiment(
        cls, experiment_id: ExperimentId = RECON_EXPERIMENT
    ) -> CorpusRecon:
        totals = mapping(
            store.load_experiment_scorecard(experiment_id)["totals"],
            field="recon.totals",
        )
        return cls(
            det_prose=integer(totals["det_prose"], field="recon.totals.det_prose"),
            disagree_prose=integer(
                totals["disagree_prose"], field="recon.totals.disagree_prose"
            ),
            disagree_lineated=integer(
                totals["disagree_lineated"], field="recon.totals.disagree_lineated"
            ),
        )
