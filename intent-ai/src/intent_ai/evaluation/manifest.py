...
from ..wire import integer, mapping, number, sequence
...
class Manifest:
    ...
    @classmethod
    def from_dict(cls, d: Mapping[str, object]) -> Self:
        prompts = mapping(d["prompts"], field="manifest.prompts")
        models = mapping(d["models"], field="manifest.models")
        return cls(
            ...
            prompts={mod: PromptFingerprint.from_dict(
                mapping(fp, field=f"manifest.prompts.{mod}")
            ) for mod, fp in prompts.items()},
            base_response_contract=str(d["base_response_contract"]),
            models={tag: str(model) for tag, model in models.items()},
            temperature=number(d["temperature"], field="manifest.temperature"),
            max_tokens=integer(d["max_tokens"], field="manifest.max_tokens"),
            reps=integer(d["reps"], field="manifest.reps"),
            seed=integer(d["seed"], field="manifest.seed"),
            price_table_version=str(d["price_table_version"]),
            sweep_axis=str(d["sweep_axis"]),
            sweep_points=tuple(str(point) for point in sequence(
                d["sweep_points"], field="manifest.sweep_points"
            )),
        )
