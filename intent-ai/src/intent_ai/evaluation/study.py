...
from ..identity import BookId, ExperimentId, Label, LineId, ReaderTag
from ..teacher import recipes, responses, tasks
from ..teacher.panel import (
    ChatCompleter,
    PanelConfig,
    ReaderConfig,
    ResponseContract,
...
def load_experiment(toml_text: str, *, prompts_dir: Path | None = None,
    ...
    meta = ExperimentMeta(
        id=ExperimentId(exp_d.get("id", base.task_id)),
        kind=str(exp_d.get("kind", "reader")),
...
def _dataset_books(name: str, *, annotations: Path | None) -> tuple[Locale, tuple[BookId, ...]]:
    ...
    langs: set[Locale] = {lid.lang for lid in ids}
    if len(langs) > 1:
...
def _base_recipe(d: Mapping[str, object], exp_d: Mapping[str, object], dataset_name: str,
                 lang: Locale, books: tuple[BookId, ...], *, prompts_dir: Path | None) -> Recipe:
    ...
    sel = mapping(d.get("selection", {}), field="experiment.selection")
    aug = dict(d)
    aug["task_id"] = str(exp_d.get("id", d.get("task_id", "experiment")))
    aug["selection"] = {
        "books": list(books),
        "target": integer(sel.get("target", 10), field="selection.target"),
        "context_radius": integer(
            sel.get("context_radius", 2), field="selection.context_radius"
        ),
        "lang": lang}
...
def _main() -> None:
    """CLI: `python -m intent_ai.evaluation.study run <experiment.toml>` — runs the study live
    ...
    toml's parent (so the durable files land beside it). A live run needs the live extra:
        `uv run --extra live python -m intent_ai.evaluation.study run <experiment.toml>`."""
    import argparse
