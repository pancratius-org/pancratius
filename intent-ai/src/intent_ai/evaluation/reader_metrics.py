...
def class_recall(votes: Iterable[PanelVote], truth: Mapping[LineId, Label],
    ...
    for lid in eval_lines:
        ...
        y_pred.append(pred if pred in ("prose", "lineated") else "__miss__")
...
def reader_cost(reps: Iterable[PanelRep], price: Price, *, n_lines: int) -> Cost:
    ...
    def token_count(value: object | None, field: str) -> int:
        return 0 if value is None else integer(value, field=field)

    for rep in reps:
        usage = rep.usage or {}
        ptok += token_count(usage.get("prompt_tokens"), "prompt_tokens")
        ctok += token_count(usage.get("completion_tokens"), "completion_tokens")
