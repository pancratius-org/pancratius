...
from .identity import BookId, BookKey, DocxPackageHash, JsonObject, JsonRow, LineId, text_hash
from .records import (
...
class ArtifactDigest:
    """Full SHA-256 of one emitted artifact member."""

    value: str

    def __post_init__(self) -> None:
        if len(self.value) != 64 or any(c not in "0123456789abcdef" for c in self.value):
            raise ValueError(f"invalid artifact SHA-256 {self.value!r}")

    @classmethod
    def of(cls, path: Path) -> ArtifactDigest:
...
class Manifest:
    ...
    book_id: BookId
    n_records: int
    records_sha256: ArtifactDigest
    schema_sha256: ArtifactDigest
    ...
    def to_dict(self) -> JsonObject:
        return {
            ...
            "records_sha256": self.records_sha256.value,
            "schema_sha256": self.schema_sha256.value,
        }
    ...
    def from_dict(cls, d: Mapping[str, object]) -> Self:
        ...
        if not is_locale(lang):
            raise ValueError(f"artifact has unsupported locale {lang!r}")
        return cls(
            producer_version=str(d["producer_version"]),
            feature_schema_version=str(d["feature_schema_version"]),
            docx_package_hash=DocxPackageHash(str(d["docx_package_hash"])),
            lang=lang,
            book_id=BookId(d["book_id"]),
            n_records=int(str(d["n_records"])),
            records_sha256=ArtifactDigest(str(d["records_sha256"])),
            schema_sha256=ArtifactDigest(str(d["schema_sha256"])),
        )

    def check(
        self,
        *,
        live_docx_hash: DocxPackageHash,
        expected_book: BookKey,
        ...
        if BookKey(self.lang, self.book_id) != expected_book:
            raise HashMismatch(
                f"artifact identity {self.lang}:{self.book_id} != expected {expected_book}"
            )
...
def emit(
    out_dir: Path, records: list[LineRecord], *, lang: Locale, book_id: BookId,
    ...
    schema_path = out_dir / SCHEMA_FILE
    write_jsonl(records_path, (r.to_dict() for r in records))
    ...
    manifest = Manifest(
        ...
        book_id,
        len(records),
        ArtifactDigest.of(records_path),
        ArtifactDigest.of(schema_path),
    )
...
def load_records(
    ...
    *,
    ...
    manifest.check(
        live_docx_hash=live_docx_hash,
        expected_book=expected_book,
        migration=migration,
    )
    if ArtifactDigest.of(records_path) != manifest.records_sha256:
        raise HashMismatch("line_records.jsonl digest does not match its manifest")
    if ArtifactDigest.of(schema_path) != manifest.schema_sha256:
        raise HashMismatch("feature_schema.json digest does not match its manifest")
    out: list[LineRecord] = []
    seen: set[LineId] = set()
    for d in read_jsonl(records_path):
        try:
            ...
            raise HashMismatch(f"invalid line record: {exc}") from exc
        if rec.id in seen:
            raise HashMismatch(f"duplicate LineId in artifact: {rec.id}")
        seen.add(rec.id)
        if rec.id.book_key != expected_book:
            raise HashMismatch(f"record {rec.id} belongs to the wrong artifact")
    ...
    stored_schema = FeatureSchema.from_dict(json.loads(schema_path.read_text()))
    if stored_schema != build_schema(out):
        raise HashMismatch("feature schema does not describe the stored records")
    return out
...
def artifact_dir(store: Path, book_id: BookId, lang: Locale) -> Path:
    return store / f"{book_id}-{lang}"


def build_records_artifact(
    docx: Path, lang: Locale, book_id: BookId, *, store: Path,
) -> list[LineRecord]:
    ...
    return load_artifact(
        out_dir,
        live_docx_hash=docx_hash,
        expected_book=BookKey(lang, book_id),
    )


def load_records_artifact(
    docx: Path,
    lang: Locale,
    book_id: BookId,
    *,
    ...
    return load_artifact(
        out_dir,
        live_docx_hash=identity.docx_package_hash(docx),
        expected_book=BookKey(lang, book_id),
    )
