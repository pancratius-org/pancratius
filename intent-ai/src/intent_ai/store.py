...
as `LineId`-key lists); interpreting them into typed records (group by reader, join slice truth,
…) is pure and lives in `annotations.py` / `evaluation.datasets`. Record
...
def _experiment_folder(experiment_id: ExperimentId, experiments: Path | None) -> Path:
    return (experiments or paths.EXPERIMENTS) / ExperimentId(experiment_id)


def write_experiment(experiment_id: ExperimentId, *, scorecard: JsonObject, report: str,
    ...
    folder = _experiment_folder(experiment_id, experiments)
...
def experiment_scorecard_path(
    experiment_id: ExperimentId, *, experiments: Path | None = None
) -> Path:
    """The path to a prior experiment's `scorecard.json` — so a downstream study can pin its sha
    (`sha256_file`) for reproducibility."""
    return _experiment_folder(experiment_id, experiments) / SCORECARD_FILE


def load_experiment_scorecard(experiment_id: ExperimentId, *,
...
def load_experiment_timestamp(experiment_id: ExperimentId, *,
    ...
    path = _experiment_folder(experiment_id, experiments) / EXPERIMENT_MANIFEST_FILE
    if not path.is_file():
        return None
...
def save_experiment_reply(experiment_id: ExperimentId, row: JsonRow, *,
...
def save_recon_rows(book_id: BookId, lang: str, rows: list[JsonRow], *,
    ...
    data — rebuilt for free by `python -m intent_ai.recon`, never committed."""
...
def load_recon_rows(book_id: BookId, lang: str, *, store: Path | None = None) -> list[JsonRow]:
    ...
    if not path.is_file():
        raise FileNotFoundError(
            f"recon rows missing: {path} — rebuild with `python -m intent_ai.recon`")
    return list(artifact.read_jsonl(path))
