...
from ..wire import mapping, string
...
def parse_ui_responses(data: JsonObject, *, tag: ReaderTag = "human") -> list[RawReaderResponse]:
    ...
    response_rows = mapping(data.get("responses", {}), field="responses")
    for item_id, raw_record in response_rows.items():
        record = mapping(raw_record, field=f"responses.{item_id}")
        lines = mapping(record.get("lines", {}), field=f"responses.{item_id}.lines")
        rows = tuple(
            RawReaderRow(key=key, label=string(value, field=f"responses.{item_id}.{key}"))
            for key, value in lines.items()
        )
        out.append(RawReaderResponse(item_id=item_id, tag=tag,
                                     rows=rows, note=string(
                                         record.get("note", ""),
                                         field=f"responses.{item_id}.note",
                                     )))
    return out
