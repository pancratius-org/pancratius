...
from ..wire import integer, mapping, number, sequence, string
...
def _opt_int(value: object) -> int | None:
    ...
    return None if value is None else integer(value, field="reasoning_max_tokens")
...
def recipe_from_dict(d: Mapping[str, object], *, prompts_dir: Path | None = None) -> Recipe:
    ...
    temperature = number(
        d.get("temperature", panel_mod.DEFAULT_TEMPERATURE), field="recipe.temperature"
    )
    max_tokens = integer(
        d.get("max_tokens", panel_mod.DEFAULT_MAX_TOKENS), field="recipe.max_tokens"
    )
    reasoning_default = d.get("reasoning_max_tokens")
    reader_rows = sequence(d.get("readers", []), field="recipe.readers")
    readers: tuple[ReaderConfig, ...] = tuple(
        ReaderConfig(
            tag=string(row["tag"], field=f"recipe.readers[{index}].tag"),
            model=string(row["model"], field=f"recipe.readers[{index}].model"),
            modality=Modality(str(row.get("modality", "text"))),
            temperature=number(
                row.get("temperature", temperature),
                field=f"recipe.readers[{index}].temperature",
            ),
            max_tokens=integer(
                row.get("max_tokens", max_tokens),
                field=f"recipe.readers[{index}].max_tokens",
            ),
            reasoning_max_tokens=_opt_int(
                row.get("reasoning_max_tokens", reasoning_default)
            ),
        )
        for index, value in enumerate(reader_rows)
        for row in (mapping(value, field=f"recipe.readers[{index}]"),)
    )
    tags = [r.tag for r in readers]
    if len(set(tags)) != len(tags):
        raise ValueError(f"duplicate reader tags in recipe: {tags}")
    sel = mapping(d["selection"], field="recipe.selection")
    books = tuple(
        BookId(value)
        for value in sequence(sel["books"], field="recipe.selection.books")
    )
    if not books:
    ...
    roster = (decision_mod.parse_roster(
        mapping(d["roster"], field="recipe.roster"), known=frozenset(tags),
                                        known_desc="the recipe's readers")
              if "roster" in d else None)
    dec = (decision_mod.policy_from_toml(mapping(d["decision"], field="recipe.decision"))
           if "decision" in d else None)
    if d.get("prompts") and "instructions" in d:
        raise ValueError("recipe gives both [prompts] and inline instructions — choose one")
    prompt_rows = mapping(d.get("prompts", {}), field="recipe.prompts")
    prompt_files = {
        key: string(value, field=f"recipe.prompts.{key}")
        for key, value in prompt_rows.items()
    }
    ...
    instructions_value = (d.get("instructions")
    ...
    instructions = string(instructions_value, field="recipe.instructions")
    contract = panel_mod.ResponseContract(str(
        d.get("contract", panel_mod.ResponseContract.JSON_ARRAY.value)
    ))
    locale = str(sel.get("lang", "ru"))
    if not is_locale(locale):
        raise ValueError(f"unsupported recipe locale {locale!r}")
    return Recipe(
        task_id=string(d["task_id"], field="recipe.task_id"),
        title=string(d.get("title", ""), field="recipe.title"),
        instructions=instructions,
        books=books,
        selector=parse_selector(str(sel.get("selector", "all"))),
        readers=readers,
        lang=locale,
        reps=integer(d.get("reps", 1), field="recipe.reps"),
        target=integer(sel.get("target", 10), field="recipe.selection.target"),
        context_radius=integer(
            sel.get("context_radius", 1), field="recipe.selection.context_radius"
        ),
        max_workers=integer(d.get("max_workers", 8), field="recipe.max_workers"),
        contract=contract,
...
def _page_size(spec: ItemSpec, records: Sequence[LineRecord], *, max_span: int,
    ...
    Every `LineId` has a source ordinal, so the split is a single ordered clustering operation."""
    pos = {r.id: i for i, r in enumerate(records)}
    votable = sorted(spec.votable, key=lambda line_id: line_id.src_ordinal)
    if not votable or votable[-1].src_ordinal - votable[0].src_ordinal <= max_span:
        return [spec]
    clusters: list[list[LineId]] = [[votable[0]]]
    for lid in votable[1:]:
    ...
    for k, cl in enumerate(clusters):
        lo = max(0, min(pos[m] for m in cl) - context_radius)
        hi = min(len(records) - 1, max(pos[m] for m in cl) + context_radius)
        out.append(ItemSpec(region_id=f"{spec.region_id}s{k}",
                            region=tuple(records[i].id for i in range(lo, hi + 1)),
                            votable=frozenset(cl)))
    return out
...
def _build_and_save(recipe: Recipe, selection: Selection, task_id: TaskId, *,
    ...
    if recipe.vision:
        if render is None:
            raise ValueError("vision recipe has no render builder")
        assets = render(specs)
    else:
        assets = {}
    if recipe.vision:
...
def _gate_label(d: LineDecision, line_votes: Mapping[ReaderTag, PanelVote], *, task_id: TaskId,
    ...
    if text_hash is None:
        raise ValueError(f"accepted line {d.id} has no canonical text hash")
    return LineLabel(
...
def _main() -> None:
    """CLI: `uv run python -m intent_ai.teacher.recipes <build|panel|route|ingest> <recipe.toml>`.
    ...
    live `panel` run needs the SDK extra:
        `uv run --extra live python -m intent_ai.teacher.recipes panel <recipe.toml>`."""
    import argparse
