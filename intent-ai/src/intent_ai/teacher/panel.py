...
from ..identity import JsonRow, ListingKey, ModelId, ReaderTag, RequestFingerprint
from ..wire import integer, mapping, string
...
def _fingerprint(messages: Sequence[Message], *, temperature: float, max_tokens: int,
    ...
    for m in messages:
        for part in m["content"]:
            if part["type"] == "text":
                h.update(b"\x00T" + part["text"].encode())
            else:
                h.update(b"\x00I" + part["image_url"]["url"].encode())
    return RequestFingerprint(h.hexdigest()[:16])
...
class CompletionRequest:
    ...
    def reply_from_row(row: JsonRow) -> ChatReply:
        ...
        usage = row.get("usage")
        return ChatReply(
            content=string(row.get("content", ""), field="reply.content"),
            finish_reason=(string(finish_reason, field="reply.finish_reason")
                           if finish_reason is not None else None),
            usage=(dict(mapping(usage, field="reply.usage")) if usage is not None else None),
        )
    ...
    def key_from_row(row: JsonRow) -> CallKey:
        ...
        return (
            string(row["item_id"], field="reply.item_id"),
            string(row["tag"], field="reply.tag"),
            integer(row["rep"], field="reply.rep"),
            string(row["model"], field="reply.model"),
            RequestFingerprint(string(row.get("prompt_hash", ""), field="reply.prompt_hash")),
        )
