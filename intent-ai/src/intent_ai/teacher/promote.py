...
a task is idempotent). `LineLabel` owns the two-class constraint and `LineId` is canonical by
construction. All IO is at the `store` edge."""
...
def promote_labels(labels: Sequence[LineLabel], *, annotations: Path | None = None) -> int:
    """Merge resolved human labels into `labels.jsonl` (one row per LineId; new wins)."""
