...
from ..identity import Label, LineId, ReaderTag, strict_bool
    ...
    def decide(self, lid: LineId, votes: Mapping[ReaderTag, PanelVote],
        ...
        if maj is None:
            return LineDecision(
                id=lid,
                outcome=Outcome.HUMAN,
                reason=Reason.NO_PANEL_MAJORITY,
                label=None,
                anchor_label=anchor.label if anchor else None,
                support_labels=support_labels,
            )
        return LineDecision(
            ...
            reason=Reason.ACCEPTED_MAJORITY,
            label=maj,
            anchor_label=anchor.label if anchor else None,
            support_labels=support_labels,
        )
...
def _anchor_led(name: str, params: Mapping[str, object]) -> DecisionPolicy:
    ...
    return AnchorLedPolicy(
        ...
        gates=AnchorLedGates(
            min_support=integer(params.get("min_support", 1), field="decision.min_support"),
            min_core_agree=integer(
                params.get("min_core_agree", 0), field="decision.min_core_agree"
            ),
            conf_floor=(number(params["conf_floor"], field="decision.conf_floor")
                        if params.get("conf_floor") is not None else None),
            require_no_split=strict_bool(
                params.get("require_no_split", True), field="decision.require_no_split"
            )))


def _equal_majority(name: str, params: Mapping[str, object]) -> DecisionPolicy:
    """`kind="equal_majority"` → the control: a strict majority of all deciding readers, anchor
    unprivileged."""
    return EqualMajorityPolicy(
        name=name,
        min_voters=integer(params.get("min_voters", 1), field="decision.min_voters"),
    )
...
def policy_from_toml(table: Mapping[str, object]) -> DecisionPolicy:
    ...
    kind = string(table["kind"], field="decision.kind")
    if kind not in POLICY_KINDS:
        raise ValueError(f"unknown policy kind {kind!r}; known: {sorted(POLICY_KINDS)}")
    params = mapping(table.get("params", {}), field="decision.params")
    return POLICY_KINDS[kind](string(table["name"], field="decision.name"), params)


def parse_roster(table: Mapping[str, object], *, known: frozenset[ReaderTag] | None = None,
    ...
    anchor = string(table["anchor"], field="roster.anchor")
    support = tuple(
        string(value, field="roster.support[]")
        for value in sequence(table.get("support", ()), field="roster.support")
    )
