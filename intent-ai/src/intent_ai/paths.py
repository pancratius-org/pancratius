...
ARTIFACT_STORE = REPO_ROOT / "intent-ai" / "_artifacts"
...
ANNOTATIONS = REPO_ROOT / "intent-ai" / "annotations"
...
TEACHER_STORE = REPO_ROOT / "intent-ai" / "_teacher"
...
CAMPAIGNS = REPO_ROOT / "intent-ai" / "campaigns"
...
def corpus_books(lang: str) -> list[BookId]:
    ...
    return sorted(
        BookId(directory.name.split("-", 1)[0])
        for directory in BOOKS.iterdir()
        if directory.is_dir() and (directory / f"{lang}.docx").is_file()
    )
