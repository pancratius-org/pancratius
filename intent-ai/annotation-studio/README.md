...
`adjudicate.html` loads a task payload built by `intent_ai.teacher` and shows, per region, the
...
`intent_ai.teacher.responses` resolves it back before anything is committed — so a source
...
1. `intent_ai.teacher.tasks` builds a task → a payload this tool loads;
2. adjudicate here → download `responses.json`;
3. `intent_ai.teacher.responses` resolves the keys to `LineId`s and `promote` commits the labels.
