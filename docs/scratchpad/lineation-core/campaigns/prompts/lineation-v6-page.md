...
page these sit together within one paragraph's spacing (no paragraph gap between them). A soft-break
group is deliberate: read the group as a DISPLAY UNIT before judging its lines.
...
UNIT BEFORE LINE
Do not classify each line in isolation. First identify the smallest DISPLAY UNIT on the page: prose
paragraph, numbered/titled item, list, litany, stanza-like run, dialogue turn. Label lines by what
that unit is doing, unless the page clearly changes register inside it.

WRAPS is prose evidence only when the whole unit reads as an ordinary flowing paragraph. It is NOT
decisive when the line is the body of a displayed unit whose title/member break is part of the page
shape. For repeated numbered/titled items, preserve the item shape: if the page presents
"1. [title]" and its following text as one displayed item, the title/body break is intentional
lineation even though the body wraps.
...
STRONG SIGNALS
  ...
  Toward PROSE:
   • Long, connected sentences that WRAP into ONE flowing paragraph — wrapping is prose evidence,
     weighed WITHIN the unit (not decisive for a wrapping body inside a titled / displayed item).
...
OUTPUT — for each keyed body line, give the label (prose | lineated) and a 0–1 confidence, under its
exact key.
